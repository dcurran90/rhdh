...
import { rhdhVaultPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [rhdhVaultPlugin],
    });

    await request(server).get('/api/rhdh-vault/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/rhdh-vault/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/rhdh-vault/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/rhdh-vault/todos/${createdTodoItem.id}`)
